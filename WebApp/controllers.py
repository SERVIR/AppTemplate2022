...
def get_timeseries_netcdf(request):
    ...
    if request.method == 'POST':
        ...
        for timestep, v in enumerate(time):
            ...
            val = np.mean(val)
            if not np.isnan(val) and val != 'Nan': # Check if the value is a number and add it to the ts_plot, the resultant array
        ...
        for dataset in DatasetType:
            ...
            if GeometryCoords[0] == '{':
                ...
                result = (climateserv.api.request_data(dataset, OperationType,
                                                       ...
                                                       SeasonalEnsemble, SeasonalVariable, 'memory_object'))  # Get the data from the ClimateServ API
            else:
            ...
            for d in data:
                ...
                time_stamp = calendar.timegm(dt.timetuple()) * 1000
                ts_plot.append([time_stamp, d['value']['avg']])  # Get the average value from the data along with the date
...
def get_timeseries_sqlite(request):
    ...
    try:
        ...
        if station != "default":
            cur.execute(
                "SELECT measurement_date,measurement_temp FROM WebApp_measurement where station_id='" + station + "' LIMIT 10") # Get the temperature data from the database
            cur1.execute(
                "SELECT measurement_date,measurement_precip FROM WebApp_measurement where station_id='" + station + "' LIMIT 10") # Get the precipitation data from the database

            rows_temp = cur.fetchall()
            rows_precip = cur1.fetchall()
            ...
            for row in rows_precip:
                dt = datetime.strptime(row[0], '%Y-%m-%d')
                time_stamp = calendar.timegm(dt.timetuple()) * 1000
                val = row[1]
                ts_plot_precip.append([time_stamp, float(val)])  # Append the data to the list
            ts_plot_temp.sort()
...
def get_gee_layer(request):
    ...
    credentials = ee.ServiceAccountCredentials(service_account, data['private_key_json'])  # private key from data.json file
    ee.Initialize(credentials)
    params = {'min': 258, 'max': 316, 'palette': ['1303ff', '42fff6', 'f3ff40', 'ff5d0f'], }
    collection = ee.ImageCollection('NASA/GLDAS/V022/CLSM/G025/DA1D').filter(ee.Filter.date('2010-06-01', '2010-06-02'))  # image collection from Google Earth Engine
...
def get_gee_user_layer(request):
    ...
    user_asset = ee.Image("projects/servir-sco-assets/assets/tmp_servir_cms/factors_t1/f2_pcp_x1k")  # user asset from Google Earth Engine
    params = {'min': 1000, 'max': 3000, 'bands': ['b1'], 'palette': ['fcffe7', 'd2ffba', '70d7ff', '423fff'], }  # parameters for the layer
